...
__global__ void Mat_Vec_Multiply_Kernel(float* d_Y, float* d_X, float* d_A, int Vec_Size)
{
	...
	for (i = 0; i < Vec_Size; i++) {
		...
		for (j = 0; j < Vec_Size; j++) {
			sum += d_A[i * Vec_Size + j] * d_X[id * Vec_Size + j];
		}
		d_Y[id * Vec_Size + i] = sum;
	}
}

void Mat_Vec_Multiply_GPU(float *pVecX, float *pMatA, float *pVecY_G)
{
	// Choose which GPU to run on, change this on a multi-GPU system.
	...
	CUDA_CALL(cudaMemcpy(d_A, pMatA, size, cudaMemcpyHostToDevice))
		...
		dim3 dimBlock(32);
	dim3 dimGrid(ELEM_PER_VECTOR * ELEM_PER_VECTOR / dimBlock.x);
	CHECK_TIME_INIT_GPU()
	CHECK_TIME_START_GPU()
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (d_Y, d_X, d_A, ELEM_PER_VECTOR);
	CHECK_TIME_END_GPU(device_time)
		CHECK_TIME_DEST_GPU()
